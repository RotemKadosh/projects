...
static state_ty CheckGetNumRes(calculator_ty *calc, char parantheses, const char **math_exp,  const char *ptr_end,int *exit_status );
/*--------------------------Definitions---------------------------*/
...
state_ty WaitForNumHendler(calculator_ty *calc, const char **math_exp, int *exit_status)
{
    ...
    {
       change_to_state = CheckGetNumRes(calc, parantheses, math_exp, ptr_end, exit_status);  
    }
...
static state_ty CheckGetNumRes(calculator_ty *calc, char parantheses, const char **math_exp,  const char *ptr_end,int *exit_status )
{
    ...
    else if(*math_exp == ptr_end) /* num == 0*/
    {
            change_to_state = ERROR;
            *exit_status = SYNTAX_ERROR;
    }
